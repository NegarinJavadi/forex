...
def apply_signal(row, trade_settings: TradeSettings):
    ...
    if row.SPREAD <= trade_settings.maxspread and row.GAIN >= trade_settings.mingain:
        if row['mid-c'] > row['BB_UP'] and row['mid-o'] < row['BB_UP']:
            return defs.SELL
        elif row['mid-c'] < row['BB_LW'] and row['mid-o'] > row['BB_LW']:
            return defs.BUY
    return defs.NONE

def apply_SL(row, trade_settings: TradeSettings):
    if row.SIGNAL == defs.BUY:
        return row['mid-c'] - (row.GAIN / trade_settings.riskreward)
...
def process_candles(df: pd.DataFrame, pair, trade_settings:TradeSettings, log_message):
    ...
    df['LOSS'] = abs(df['mid-c'] - df.SL)

    log_cols = ['PAIR', 'time', 'mid-c', 'mid-o', 'SL', 'TP', 'SPREAD', 'GAIN', 'LOSS', 'SIGNAL']
