...
class OpenFxApi:
    ...
    def save_response(self, resp, filename):
        with open(f'./openfx_api/api_data/{filename}.json', 'w') as f:
            d = {}
            d['local_request_data'] = dt.datetime.now().strftime('%Y-%m-%d %H:%M:%S')
    ...
    def make_request(self, url, verb='get', code=200, params=None, data=None, headers=None, save_filename=""):
        ...
        try:
            ...
            if response.status_code == code:
                ...
                print(f"Request to {full_url} failed with status code {response.status_code} and response {response.text}")
                return False, response.json()
        except Exception as error:
            print(f"Exception in make_request: {error}")
    ...
    def get_account_instruments(self, StatusGroupId='Forex'):
        ...
        if not ok:
            print("ERROR fetching historical symbols", his_symbol_data)
            return None
    ...
    def fetch_candles(self, pair_name, count=-10, granularity="H1", ts_f=None):
        available_symbols = [symbol['Symbol'] for symbol in self.get_account_instruments()]
        print("Available symbols:", available_symbols)  # Print available symbols for debugging
        if pair_name not in available_symbols:
            print(f"Symbol {pair_name} not found in available symbols.")
            return False, None

        if ts_f is None:
            ts_f = int(pd.Timestamp(dt.datetime.now(dt.timezone.utc)).timestamp() * 1000)
        ...
        try:
            ...
            if ok_ask and ok_bid:
                ...
                print(f"Failed to fetch candles. ok_ask: {ok_ask}, ok_bid: {ok_bid}")
                return False, None

        except Exception as e:
            print(f"Exception in fetch_candles: {e}")
            return False, None
    ...
    def get_candles_df(self, pair_name, count=-10, granularity="H1", date_f=None):
        if date_f is None:
            date_f = int(pd.Timestamp.now(tz=dt.timezone.utc).timestamp() * 1000)
        else:
...
class OpenFxApi:
    ...
    def get_account_instruments(self, StatusGroupId='Forex'):
        ...
        url = f"quotehistory/symbols"
        ok, his_symbol_data = self.make_request(url, save_filename="quotehistoy_symbols")
